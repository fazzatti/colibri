...
export const extractContractSpec = (wasm: BinaryData): ContractSpec => {
  ...
  try {
    ...
    throw new ERROR.INVALID_WASM_FOR_SPEC(cause as Error);
  }

  const sections = WebAssembly.Module.customSections(module, "contractspecv0");
  if (sections.length === 0) throw new ERROR.MISSING_SPEC_IN_WASM();
  ...
  for (const [sectionIndex, section] of sections.entries()) {
    try {
      ...
      throw new ERROR.FAILED_TO_DECODE_SPEC_SECTION(
        sectionIndex,
        cause as Error,
      );
    }
