...
export class Operation {
  ...
  @memoize()
  get type(): string {
    return this.rawOperation.body.type;
  }
  ...
  get body(): unknown {
    ...
    switch (opBody.type) {
      ...
      default:
        throw new UNSUPPORTED_OPERATION_TYPE(
          (opBody as { type: string }).type,
        );
    }
  }
  ...
  private parseSetOptions(op: xdr.SetOptionsOp): unknown {
    const signerKey = op.signer?.key.value;
    return {
  ...
  private parseManageData(op: xdr.ManageDataOp): unknown {
    return {
      ...
      dataValue: op.dataValue ? toHex(op.dataValue.toBytes()) : null,
    };
  }
  ...
  private parseClaimClaimableBalance(
    ...
    return {
      balanceId: toHex(op.balanceId.value.toBytes()),
    };
  }
  ...
  private parseLiquidityPoolDeposit(
    ...
    const poolIdHex = toHex(op.liquidityPoolId.toBytes());
    return {
