import { AddressError } from "@/address/error.ts";

import { regex } from "@/common/index.ts";
import type { MuxedAddress } from "@/strkeys/types.ts";

export enum Code {
  INVALID_MUXED_ADDRESS = "ADDR_MTBA_001",
  FAILED_TO_LOAD_MUXED_ACCOUNT_FROM_ADDRESS = "ADDR_MTBA_002",
  FAILED_TO_RETRIEVE_THE_BASE_ACCOUNT_ID = "ADDR_MTBA_003",
}
...
export abstract class MuxedAddressToBaseAccountError extends AddressError<
  Code,
  MetaData
> {
  override readonly source = "@colibri/core/address/muxed-to-base-account";
}
