...
adapters remain in [`@colibri/react`](../react.md), under separate public
entrypoints:
...
- Required `options.capabilities({ module, address, networkPassphrase })`
  ...
  capabilities and `messageSigner` for
  [SEP-53](../../core/signer/message-signing.md). Omitted capabilities stay
  absent; method presence and wallet names do not establish support.
...
- State changes, wallet selection and Kit disconnect immediately clear the
  Colibri connection and release its listeners. Call
  [`useReconnect`](hooks/use-reconnect.md) explicitly after a state change to
  reread the cached account and network without a prompt. Use an explicit
  Connect action after selecting another wallet: Kit can retain the previous
  wallet's cached address, so that value cannot establish the new connection.
...
selected wallets. Core's
[`AuthEntrySigner`](../../core/signer/README.md#signer-capabilities) transforms
a complete authorization entry; the upstream wallet method accepts an
authorization preimage and returns a signature. Supply an explicitly adapted
Core signer when needed. The same applies to message encoding and
[SEP-53](../../core/signer/message-signing.md) semantics. Application-provided
signers retain their original Core contract;
[`useSigners`](hooks/use-signers.md) and
[`useSignMessage`](hooks/use-sign-message.md) add the provider's connection
guards.
...
Call [`useConnect()("freighter")`](hooks/use-connect.md) from a user action.
Reconnect reads without prompting. Account/network changes are polled;
disconnect stops polling and clears Colibri authority, without revoking the
extension's website permission. Each envelope signature checks identity before
and after the prompt and verifies the returned signer address. This adapter
exposes envelope signing only.
...
signer, independently of any controlled account. Supply Core
[`AuthEntrySigner`](../../core/signer/README.md#signer-capabilities) or
[`MessageSigner`](../../core/signer/message-signing.md) capabilities separately
when available. A contract account does not imply envelope-signing authority.

[Generic wallet API](https://jsr.io/@colibri/react/doc/wallets).

## WebAuth

[`useWebAuthClient(domain)`](hooks/use-web-auth-client.md) discovers a client
through [SEP-1](../../core/sep1.md) and exposes ordinary query state. An
application can instead construct an existing
[`WebAuthClient`](../webauth/discovery.md). Create the session outside rendering
once that client is available:
...
Within the same provider, [`useSession(session)`](hooks/use-session.md) returns
the current state and [`useWebAuth(session)`](hooks/use-web-auth.md) returns an
authentication mutation. Call `authenticate.mutateAsync({ account, signer })`
for [SEP-10](../webauth/sep10.md) with an SDK keypair, Core local signer, or
asynchronous Core envelope signer, or supply the existing WebAuth
[SEP-45](../webauth/sep45.md) `authorize` options for a contract account. See
...
guarded wallet signers. A session aborts pending [SEP-10](../webauth/sep10.md)
signing on logout, disconnect, account/network changes and disposal, before
exchanging a late approval. It cannot dismiss wallet prompts or recall an
exchange already sent.

Read the token from `session.getSnapshot().token` or
[`useSession`](hooks/use-session.md); it is not placed in query/mutation result
data. Token decoding alone cannot establish a session. Tokens returned by the
completed exchange must be current and bound to the expected account and home
domain. Use the token only with the intended service; the server remains
...
[`useStellarToml(domain, options, query, scope)`](hooks/use-stellar-toml.md)
returns the full [SEP-1](../../core/sep1.md) facade, including currency and
service advertisements. Different custom fetchers or validation policies need
distinct scopes. [`useIdenticon(address)`](hooks/use-identicon.md) returns a
local SVG data URL. [`AccountIdenticon`](hooks/use-identicon.md) requires
explicit `alt` text (empty for decoration); it uses the SVG-only renderer, with
no PNG encoder in that import.
