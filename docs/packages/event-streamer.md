...
// Stream for next 10 ledgers
const latestLedger = await streamer.getLatestLedger();

await streamer.start(
  (event) => {
    const transfer = SACEvents.TransferEvent.fromEvent(event);
    console.log(
      `[Ledger ${transfer.ledger}] ${transfer.from} → ${transfer.to}`
    );
  },
  {
    stopLedger: latestLedger + 10,
  }
);
...
const filter = new EventFilter({
  contractIds: ["CB23WRDQWGSP6YPMY4UV5C4OW5CBTXKYN3XEATG7KJEZCXMJBYEHOUOV"],
  type: EventType.Contract,
...
await streamer.start(
  (event) => {
    const mint = SACEvents.MintEvent.fromEvent(event);
    console.log(`Mint #${++count}: ${mint.amount} to ${mint.to}`);
  },
  {
    startLedger: 59895694,
    stopLedger: 59895694,
...
console.log(`Processed ${count} mint events`);
```

### Continuous Monitoring with CAP-67
