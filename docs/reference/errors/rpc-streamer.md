...
| `RPC_025` | `NETWORK_DISCOVERY_FAILED` — Failed to resolve network identity from the ledger RPC connection. | [Definition](https://github.com/fazzatti/colibri/blob/main/rpc-streamer/src/errors.ts#L80) |
| `RPC_026` | `INVALID_NETWORK_PASSPHRASE` — Network discovery returned an empty or malformed passphrase.     | [Definition](https://github.com/fazzatti/colibri/blob/main/rpc-streamer/src/errors.ts#L82) |
