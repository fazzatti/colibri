...
Submits a signed transaction to the network and waits for confirmation. This
process handles the full lifecycle of transaction submission, including polling
...
3. **Calculates wait time** — If `useTransactionTimeoutIfAvailable` is true and
   the transaction has a timeout, uses that; otherwise uses `timeoutInSeconds`
4. **Polls for status** — Recursively calls `rpc.getTransaction()` until:
   - Transaction reaches a final status (`SUCCESS` or `FAILED`)
   - Timeout is reached
5. **Returns result** — On success, returns hash, return value, and full
   response
