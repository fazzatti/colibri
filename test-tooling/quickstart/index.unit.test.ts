...
describe("StellarTestLedger unit", () => {
  it("constructor validates supported options", () => {
  ...
  it("constructor validates remaining service and storage edge cases", () => {
  ...
  it("constructor normalizes empty container identifiers and returns fresh storage defaults", () => {
  ...
  it("constructor rejects non-string container identifiers", () => {
  ...
  it("constructor preserves numeric TRACE log levels", () => {
  ...
  it("constructor reuses the same Docker client instance", () => {
    ...
    assertStrictEquals(
      ledger.exposeDockerClient(),
      ledger.exposeDockerClient(),
    );
  });

  it("constructor accepts arbitrary image tags through containerImageVersion", () => {
    const ledger = new StellarTestLedger({
    ...
    assertEquals(
      ledger.fullContainerImageName,
      "stellar/quickstart:v999-custom",
    );
  });

  it("constructor accepts non-local networks and persistent storage", () => {
  ...
  it("getContainer throws before the ledger starts", () => {
  ...
  it("start creates a named container and exposes network information", async () => {
    const harness = createDockerHarness();
  ...
  it("start uses the selected network, services, and storage mode", async () => {
    const harness = createDockerHarness();
    const ledger = new TestLedger(
      {
        network: NetworkEnv.TESTNET,
  ...
  it("start can skip pull and optionally stream logs", async () => {
    const harness = createDockerHarness();
    const ledger = new TestLedger(
  ...
  it("start reuses a running named container when useRunningLedger is enabled", async () => {
    const harness = createDockerHarness();
  ...
  it("start can reuse a running named container without published ports", async () => {
    const harness = createDockerHarness();
    const existing = createMockContainer(
      createInspectInfo({
        NetworkSettings: {
  ...
  it("start rejects when the running ledger cannot be reused safely", async () => {
  ...
  it("start removes stale named containers and existing tracked containers before recreating", async () => {
    ...
    const trackedLedger = new TestLedger(
      undefined,
      trackedHarness.dockerClient,
    );
    const previous = createMockContainer(
      createInspectInfo({ Id: "previous" }),
      {
        id: "previous",
      },
    );
  ...
  it("start removes a tracked container without an id using direct Docker cleanup", async () => {
    const harness = createDockerHarness();
    const ledger = new TestLedger(undefined, harness.dockerClient);
    const previous = createMockContainer(
      createInspectInfo({ Id: "ephemeral" }),
      {
        omitId: true,
      },
    );
  ...
  it("getNetworkDetails uses the Docker daemon host for remote connections", async () => {
    const harness = createDockerHarness();
    const ledger = new TestLedger(
  ...
  it("getNetworkConfiguration delegates to getNetworkDetails", async () => {
    const harness = createDockerHarness();
    const ledger = new TestLedger(undefined, harness.dockerClient);
  ...
  it("getNetworkDetails follows the selected network and service tuple", async () => {
    const harness = createDockerHarness();
    ...
    assertEquals(await futureLedger.getNetworkDetails(), {
      ...
      transactionsExplorerUrl:
        "http://127.0.0.1:18000/lab/transactions-explorer",
      allowHttp: true,
    });
  });

  it("start rejects on stale named container image mismatches and pull failures", async () => {
  ...
  it("start cleans up a newly created container when startup readiness fails", async () => {
  ...
  it("start cleans up a newly created container without an id when startup fails", async () => {
    const harness = createDockerHarness();
  ...
  it("start preserves the startup error when cleanup also fails", async () => {
    const harness = createDockerHarness();
    const created = createMockContainer(
  ...
  it("public APIs wrap unexpected underlying failures in quickstart errors", async () => {
    const harness = createDockerHarness();
    const ledger = new TestLedger(undefined, harness.dockerClient);
    ...
    const inspectFailureLedger = new TestLedger(
      undefined,
      harness.dockerClient,
    );
  ...
  it("stop and destroy are no-ops when there is nothing to do", async () => {
    const harness = createDockerHarness();
    const ledger = new TestLedger(undefined, harness.dockerClient);
  ...
  it("stop and destroy act on the tracked container", async () => {
    const inspectInfo = createInspectInfo({
      Id: "tracked",
  ...
  it("destroy removes a tracked container without an id", async () => {
    const harness = createDockerHarness();
    const ledger = new TestLedger(undefined, harness.dockerClient);
    const tracked = createMockContainer(
      createInspectInfo({ Id: "ephemeral" }),
      {
        omitId: true,
      },
