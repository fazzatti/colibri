...
Deno.test("routing fully validates supported addresses", () => {
...
Deno.test("error hierarchy carries protocol-safe metadata", () => {
...
Deno.test("WebAuthToken decode mode exposes claims without auth context", () => {
...
Deno.test("WebAuthToken validates authenticated SEP context", () => {
  const context = {
...
Deno.test("WebAuthToken rejects malformed tokens and context mismatches", () => {
...
Deno.test("WebAuthTransport sends exact GET and JSON/form POST requests", async () => {
...
Deno.test("WebAuthTransport maps status, parsing, shape, network, and timeout failures", async () => {
