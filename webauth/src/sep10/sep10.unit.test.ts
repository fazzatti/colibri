...
Deno.test("verifySep10Challenge accepts strict inclusive challenge boundaries", () => {
...
Deno.test("verifySep10Challenge binds memo and an accepted client domain", () => {
  const fixture = createWebAuthFixture();
...
Deno.test("verifySep10Challenge maps XDR, sequence, source, and time failures", () => {
  const fixture = createWebAuthFixture();
  expectCode(
    () =>
      verifySep10Challenge({
...
Deno.test("verifySep10Challenge rejects malformed first operations", () => {
  const fixture = createWebAuthFixture();
...
Deno.test("verifySep10Challenge rejects memo and later-operation mismatches", () => {
  const fixture = createWebAuthFixture();
  for (
...
Deno.test("verifySep10Challenge enforces client-domain and server signatures", () => {
  const fixture = createWebAuthFixture();
...
Deno.test("Sep10Client completes an immutable account-only flow", async () => {
  const fixture = createWebAuthFixture();
...
Deno.test("Sep10Client client-domain flow fetches TOML only when accepted", async () => {
  const fixture = createWebAuthFixture();
  const now = Math.floor(Date.now() / 1_000);
...
Deno.test("Sep10Client rejects request, discovery, signing, state, and token failures", async () => {
  const fixture = createWebAuthFixture();
...
Deno.test("Sep10Client requires and validates an accepted client-domain signer", async () => {
  const fixture = createWebAuthFixture();
  const challengeXdr = buildSep10Challenge(fixture, {
    clientDomain: "wallet.test",
  });
