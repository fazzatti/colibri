...
Deno.test("SEP-45 challenge states expose defensive XDR clones", async () => {
  ...
  assertNotStrictEquals(challenge.verified.entries, challenge.verified.entries);
...
Deno.test("ContractAuth adapters preserve the full-entry callback boundary", async () => {
  const fixture = createWebAuthFixture();
...
Deno.test("Sep45Client authorizes the whole returned entry without comparison", async () => {
  const fixture = createWebAuthFixture();
...
Deno.test("Sep45Client validates authorization state, handler, validity, and expiry", async () => {
  const fixture = createWebAuthFixture();
  const challenge = await challengeState(fixture);
  const client = clientFor(
    fixture,
...
Deno.test("Sep45Client signs accepted client-domain entries", async () => {
  const fixture = createWebAuthFixture();
...
Deno.test("validateSep45Footprint accepts only SEP-defined writes", () => {
  const fixture = createWebAuthFixture();
...
Deno.test("simulateSep45Challenge enforces mode, expiry, footprint, and restoration", async () => {
  const fixture = createWebAuthFixture();
  const challenge = await challengeState(fixture);
  const authorized = new Sep45AuthorizedChallenge(
    challenge.verified,
...
Deno.test("Sep45Client completes request, authorization, simulation, and submission", async () => {
  const fixture = createWebAuthFixture();
...
Deno.test("Sep45Client performs client-domain discovery only when accepted", async () => {
  const fixture = createWebAuthFixture();
  const now = Math.floor(Date.now() / 1_000);
...
Deno.test("Sep45Client maps request, discovery, RPC, state, and token failures", async () => {
  const fixture = createWebAuthFixture();
  const challenge = await buildSep45Challenge(fixture);
