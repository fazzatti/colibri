...
Deno.test("SEP-45 codecs preserve exact variable-length entry arrays", async () => {
...
Deno.test("verifySep45Challenge accepts reordered roles, arguments, and extensions", async () => {
  const fixture = createWebAuthFixture();
  const values = {
...
Deno.test("verifySep45Challenge accepts and binds a client domain", async () => {
  const fixture = createWebAuthFixture();
  const challenge = await buildSep45Challenge(fixture, {
    clientDomain: true,
...
Deno.test("verifySep45Challenge rejects unsupported credential variants", async () => {
  const fixture = createWebAuthFixture();
  const challenge = await buildSep45Challenge(fixture);
...
Deno.test("verifySep45Challenge rejects malformed invocations", async () => {
  const fixture = createWebAuthFixture();
  const challenge = await buildSep45Challenge(fixture);
...
Deno.test("verifySep45Challenge rejects malformed and inconsistent maps", async () => {
  const fixture = createWebAuthFixture();
  const challenge = await buildSep45Challenge(fixture);
  const entries = decodeSep45AuthorizationEntries(challenge.xdr);
...
Deno.test("verifySep45Challenge binds every required argument", async () => {
  const fixture = createWebAuthFixture();
...
Deno.test("verifySep45Challenge enforces required unique roles", async () => {
  const fixture = createWebAuthFixture();
  const challenge = await buildSep45Challenge(fixture);
  const entries = decodeSep45AuthorizationEntries(challenge.xdr);
  for (
...
Deno.test("verifySep45Challenge enforces client-domain pairing and role", async () => {
  const fixture = createWebAuthFixture();
  const cases: Array<{
...
Deno.test("verifySep45Challenge verifies server signature and expiration", async () => {
  const fixture = createWebAuthFixture();
  const challenge = await buildSep45Challenge(fixture);
  const entries = decodeSep45AuthorizationEntries(challenge.xdr);
...
Deno.test("verifySep45Challenge rejects duplicate client-domain roles", async () => {
  const fixture = createWebAuthFixture();
