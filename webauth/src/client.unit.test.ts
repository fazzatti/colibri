...
describe("WebAuthClient", () => {
  it("WebAuthClient exposes complete direct protocol configuration", () => {
    ...
    assertEquals(
      client.sep10.endpoint,
      `https://${fixture.webAuthDomain}/sep10`,
    );
    assertEquals(
      client.sep45.endpoint,
      `https://${fixture.webAuthDomain}/sep45`,
    );
  ...
  it("WebAuthClient validates direct configuration without network calls", () => {
    const fixture = createWebAuthFixture();
    const network = NetworkConfig.TestNet();
    for (
      const config of [
    ...
    for (
      const endpoint of ["not-a-url", "ftp://auth.test", "http://auth.test"]
    ) {
      expectCode(
        () =>
          new WebAuthClient({
  ...
  it("WebAuthClient constructs from complete TOML and enforces network", () => {
    const fixture = createWebAuthFixture();
  ...
  it("WebAuthClient discovers TOML through the configured fetch boundary", async () => {
    const fixture = createWebAuthFixture();
  ...
  it("WebAuthClient rejects protocol-incompatible automatic options", async () => {
    const fixture = createWebAuthFixture();
