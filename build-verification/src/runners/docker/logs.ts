...
const hasDockerMultiplexedHeader = (bytes: Uint8Array): boolean =>
  bytes.length >= 8 && (bytes[0] === 1 || bytes[0] === 2) &&
  bytes[1] === 0 && bytes[2] === 0 && bytes[3] === 0;

const decodeMultiplexedDockerLogs = (
  bytes: Uint8Array,
...
export const demultiplexDockerLogs = (
  ...
  try {
    ...
    if (!hasDockerMultiplexedHeader(bytes)) {
      return { stdout: boundedText(bytes, maximum), stderr: "" };
    }
    const decoded = decodeMultiplexedDockerLogs(bytes);
    return {
      stdout: boundedText(decoded.stdout, maximum),
      stderr: boundedText(decoded.stderr, maximum),
    };
  } catch (cause) {
